/* Roundtrip test for postgres.upsert + BYTEA columns.
 *
 * Drives a pg.read → pg.upsert pipeline against a source table whose
 * `payload` column is BYTEA. Verification uses SQL EXCEPT (BYTEA is
 * comparable). Three rows: random bytes, a single-byte 0x00 cell, and
 * NULL — the zero byte is the awkward one for any C string handling.
 */
...
int main(void) {
    ...
    char schema[64];
    snprintf(schema, sizeof schema, "betl_bin_%d", (int)getpid());
    char ddl[512];
    ...
    snprintf(ddl, sizeof ddl,
        "CREATE TABLE %s.src (id BIGINT PRIMARY KEY, payload BYTEA)", schema);
    if (pg_exec(c, ddl) != 0) { PQfinish(c); return 1; }
    snprintf(ddl, sizeof ddl,
        "CREATE TABLE %s.dst (id BIGINT PRIMARY KEY, payload BYTEA)", schema);
    if (pg_exec(c, ddl) != 0) { PQfinish(c); return 1; }
    /* Row 2's `\x00` is the awkward case — would break a C-string
     * round-trip but works under length-tracked binary. */
    snprintf(ddl, sizeof ddl,
    ...
    snprintf(yaml, sizeof yaml,
        ...
        "        query: SELECT id, payload FROM %s.src ORDER BY id\n"
        "      - id: sink\n"
        ...
        schema, schema);
    char path[64];
    snprintf(path, sizeof path, "/tmp/betl-test-pg-upsert-binary-%d.yml", (int)getpid());
    ...
    snprintf(sql, sizeof sql,
        "SELECT count(*) FROM ("
        "  SELECT id, payload FROM %s.src EXCEPT "
        "  SELECT id, payload FROM %s.dst"
        ") d", schema, schema);
    PGresult *r = PQexec(c, sql);
    ...
    printf("ok: pg_upsert binary roundtrip test passed\n");
    return 0;
}
