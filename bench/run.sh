...
mkdir -p "$(dirname "$out_md")"
{
    ...
    echo
    echo "## SSIS comparison"
    echo
    echo "A measured end-to-end wall comparison against real"
    echo "SSIS (SQL Server 2022 dtexec on Linux, via the"
    echo "\`mcp-ssis\` service) lives in \`docs/BENCHMARKS.md\`"
    echo "under \"SSIS comparison (real measured numbers)\"."
    echo "Quick summary at 100k rows on a stock read+derived"
    echo "10-col workload: betl is 30–96× faster end-to-end;"
    echo "at 1M rows (steady-state, where SSIS startup is"
    echo "amortised) it's still ~5.8× faster per row."
    echo
    echo "What's still untested: real SSIS Script Components"
    echo "/ custom managed \`PipelineComponent\` subclasses,"
    echo "which the Linux SSIS subset doesn't ship. That A/B"
    echo "needs Windows + SSDT — see the limitations note in"
